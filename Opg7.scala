...
// Please note** I am using bump instead of hump because my dataset did not have any occurences of hump.
// I wanted some output to prove that it works, and I'm using bump as a substitute because it has the exact same format
// as hump, therefore the logic of the solution is the same
val nodeListWithTrafficCalmingHump = nodeListExploded.where("_k == 'traffic_calming' AND _v == 'bump'")
